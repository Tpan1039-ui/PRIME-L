...
- GEMME pseudo-fitness data for the LPE1439 auxiliary meta-learning task described in the manuscript.
...
The manuscript uses GEMME scores for all possible LPE1439 single-amino-acid
substitutions as an additional pseudo-labeled auxiliary task. PRIME-L accepts
GEMME data through `--gemme-data` or the `gemme_data` field in
`configs/default.json`.
...
LPE1439 wild-type sequence parsed from the study workbook.
...
matching the meta-learning setup described in the manuscript.
...
- `--gemme-data`: GEMME pseudo-fitness CSV, TSV, TXT, or Excel file for LPE1439 single-site substitutions. This is the manuscript-style third auxiliary task.
...
- `--pseudo-task-size`: optional fallback that creates a PRIME zero-shot pseudo task. The manuscript-style setting should use `--gemme-data` instead.
